...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        ...
        <div class="flex items-baseline">
            <span class="text-3xl font-bold text-gray-900">{{ array_sum($userCounts) }}</span>
            <span class="text-gray-500 ml-2">users</span>
        </div>
        <div class="mt-4 flex flex-wrap gap-2">
            <span class="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded">
                {{ $userCounts['tenants'] }} Tenants
            </span>
            <span class="bg-green-100 text-green-800 text-xs font-medium px-2.5 py-0.5 rounded">
                {{ $userCounts['landlords'] }} Landlords
            </span>
            <span class="bg-purple-100 text-purple-800 text-xs font-medium px-2.5 py-0.5 rounded">
                {{ $userCounts['agents'] }} Agents
            </span>
            <span class="bg-gray-100 text-gray-800 text-xs font-medium px-2.5 py-0.5 rounded">
                {{ $userCounts['admins'] }} Admins
            </span>
        </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        ...
        <div class="flex items-baseline">
            <span class="text-3xl font-bold text-gray-900">{{ $pendingVerifications + $pendingDocuments }}</span>
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        ...
        <div class="flex items-baseline">
            <span class="text-3xl font-bold text-gray-900">{{ $disputedReviews }}</span>
            <span class="text-gray-500 ml-2">{{ Str::plural('dispute', $disputedReviews) }}</span>
        </div>
        <div class="mt-4">
            <a href="{{ route('admin.reviews.disputes') }}" class="text-red-600 hover:text-red-800 text-sm font-medium">
                Resolve disputes
            </a>
        </div>
    </div>
...
<div class="mb-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <a href="{{ route('admin.verifications') }}" class="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow flex items-center">
            <div class="mr-4">
                <span class="h-10 w-10 rounded-full bg-yellow-100 flex items-center justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-yellow-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
        ...
        <a href="{{ route('admin.documents') }}" class="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow flex items-center">
            <div class="mr-4">
                <span class="h-10 w-10 rounded-full bg-blue-100 flex items-center justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <a href="{{ route('admin.reviews.disputes') }}" class="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow flex items-center">
            <div class="mr-4">
                <span class="h-10 w-10 rounded-full bg-red-100 flex items-center justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-red-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<div class="bg-white p-6 rounded-lg shadow-md mb-6">
    ...
    <div class="mt-4 text-right">
        <a href="{{ route('admin.analytics') }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium">
