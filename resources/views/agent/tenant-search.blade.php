...
<div class="bg-white p-6 rounded-lg shadow-md">
    <form method="GET" action="{{ route('agent.tenant.search') }}" class="mb-8">
        <div class="flex">
        ...
        @else
            <div class="mt-4">
                <label for="property_id" class="block text-gray-700 text-sm font-bold mb-2">Select Property (Optional)</label>
                <select id="property_id" name="property_id" class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                    <option value="">-- Select a property for quick add --</option>
                    @foreach($agent->landlords()->wherePivot('verified', true)->get() as $landlord)
                        @foreach($landlord->properties as $property)
                            <option value="{{ $property->id }}" {{ request('property_id') == $property->id ? 'selected' : '' }}>
                                {{ $property->name }} ({{ $property->city }}) - Owner: {{ $landlord->name }}
                            </option>
                        @endforeach
                    @endforeach
                </select>
                    ...
                    <div class="border rounded-lg overflow-hidden">
                        <div class="bg-gray-50 p-4">
                            <div class="flex items-center">
                                <div class="h-12 w-12 rounded-full bg-green-100 flex items-center justify-center mr-4 relative">
                                    <span class="text-xl font-bold text-green-600">{{ substr($tenant->name, 0, 1) }}</span>
                        ...
                        <div class="bg-gray-50 p-4 border-t">
                            @if(request('property_id'))
                                ...
                                <div id="add-tenant-modal-{{ $tenant->id }}" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
                                    <div class="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
                                        <h3 class="text-lg font-bold mb-4">Add {{ $tenant->name }} to Property</h3>
                                        ...
                                        @if($property)
                                            <div class="mb-4">
                                                <p class="font-medium">Property:</p>
                                                <p>{{ $property->name }} - {{ $property->address }}, {{ $property->city }}, {{ $property->state }}</p>
                                            </div>
                                            
                                            <form method="POST" action="{{ route('agent.tenant.add', ['tenant' => $tenant->id, 'property' => $property->id]) }}">
                                                @csrf
                                        ...
                                        @else
                                            <div class="bg-yellow-50 p-4 rounded-md mb-4">
                                                <div class="flex">
                                                    ...
                                                    <div class="ml-3">
                                                        <h3 class="text-sm font-medium text-yellow-800">Property Not Found</h3>
                                                        <div class="mt-2 text-sm text-yellow-700">
                                                            <p>The selected property was not found. Please try again.</p>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            @else
                                <button type="button" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
                                        onclick="document.getElementById('select-property-modal-{{ $tenant->id }}').classList.remove('hidden')">
                                    Add to Property
                                </button>
                                
                                <!-- Select Property Modal -->
                                <div id="select-property-modal-{{ $tenant->id }}" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
                                    <div class="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
                                        ...
                                        @if($availableProperties->count() > 0)
                                            <form method="POST" action="{{ route('agent.tenant.add', ['tenant' => $tenant->id, 'property' => '__property_id__']) }}" id="add-tenant-form-{{ $tenant->id }}">
                                                @csrf
                                                    ...
                                                    <select id="property-{{ $tenant->id }}" class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required
                                                        ...
                                                        @foreach($availableProperties as $property)
                                                            <option value="{{ $property['id'] }}">
                                                                {{ $property['name'] }} - {{ $property['city'] }}, {{ $property['state'] }} (Owner: {{ $property['landlord_name'] }})
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                ...
                                                <div class="flex justify-end">
                                                    <button type="button" class="mr-2 px-4 py-2 bg-gray-200 text-gray-800 rounded hover:bg-gray-300"
                                                            onclick="document.getElementById('select-property-modal-{{ $tenant->id }}').classList.add('hidden')">
                                                        Cancel
                                                    </button>
                                                    <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
                                                        Add Tenant
                                            ...
                                            <div class="bg-yellow-50 p-4 rounded-md mb-4">
                                                <div class="flex">
                                                    ...
                                                    <div class="ml-3">
                                                        ...
                                                        <div class="mt-2 text-sm text-yellow-700">
                                                            <p>You need to connect with landlords who have properties before you can add tenants.</p>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="mt-4">
                                                    <a href="{{ route('agent.landlords') }}" class="text-sm font-medium text-yellow-800 hover:text-yellow-900">
