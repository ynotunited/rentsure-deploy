...
<div class="px-4 py-6 max-w-md mx-auto">
        ...
        <div class="p-4">
            <form action="{{ route('landlord.tenant.search') }}" method="GET" class="space-y-4">
                ...
                <div class="flex flex-col space-y-3">
                    ...
                    @if(request()->hasAny(['name', 'phone', 'nin']))
                        <a href="{{ route('landlord.dashboard') }}" class="text-center text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 text-sm">Clear Search</a>
                    @endif
                </div>
            </form>
            ...
            @if(isset($searchResults))
                <div class="mt-6 border-t pt-6">
                    <h3 class="text-lg font-medium text-gray-900 mb-4">Search Results ({{ $searchResults->count() }})</h3>
                                    ...
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center space-x-4">
                                            ...
                                            <div>
                                                ...
                                                @if($tenant->state)
                                                    <p class="text-sm text-gray-500">{{ $tenant->state }}</p>
                                                @endif
                                            </div>
                                        </div>
                                        <div class="flex items-center space-x-4">
                                            ...
                                            @endif
                                            <a href="{{ route('landlord.tenant.profile', $tenant) }}" class="bg-blue-600 text-white px-4 py-2 rounded-md text-sm hover:bg-blue-700">
                                                View Profile
                                            </a>
                                        </div>
                                    </div>
    ...
    <div class="grid grid-cols-1 gap-4 mb-6">
        ...
        <div class="flex items-baseline">
            <span class="text-3xl font-bold text-gray-900">{{ $propertyCount }}</span>
            <span class="text-gray-500 ml-2">{{ Str::plural('property', $propertyCount) }}</span>
        </div>
        <div class="mt-4">
            <a href="{{ route('landlord.properties') }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                View all properties
            </a>
        </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        ...
        <div class="flex items-baseline">
            ...
            <span class="text-gray-500 ml-2">active {{ Str::plural('tenant', $tenantCount) }}</span>
        </div>
        <div class="mt-4">
            <a href="{{ route('landlord.tenants') }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                Manage tenants
            </a>
        </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        ...
        <div class="flex items-baseline">
            <span class="text-3xl font-bold text-gray-900">{{ $pendingAgentReviews }}</span>
            <span class="text-gray-500 ml-2">pending {{ Str::plural('review', $pendingAgentReviews) }}</span>
        </div>
        <div class="mt-4">
            <a href="{{ route('landlord.agent-reviews') }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium">
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        ...
        <div class="flex items-baseline">
            <span class="text-3xl font-bold text-gray-900">{{ $verifiedAgentCount }}</span>
            <span class="text-gray-500 ml-2">verified {{ Str::plural('agent', $verifiedAgentCount) }}</span>
        </div>
        <div class="mt-4">
            <a href="{{ route('landlord.agents') }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                Manage agents
            </a>
        </div>
    </div>
...
<div class="mb-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <a href="{{ route('landlord.property.create') }}" class="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow flex items-center">
            <div class="mr-4">
        ...
        <a href="{{ route('landlord.tenant.search') }}" class="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow flex items-center">
            <div class="mr-4">
                <span class="h-10 w-10 rounded-full bg-green-100 flex items-center justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <a href="{{ route('landlord.agent-reviews') }}" class="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow flex items-center">
            <div class="mr-4">
                <span class="h-10 w-10 rounded-full bg-yellow-100 flex items-center justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-yellow-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<div class="bg-white p-6 rounded-lg shadow-md">
    ...
    @if($landlord->properties->count() > 0)
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach($landlord->properties->take(5) as $property)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-500">{{ $property->activeTenants()->count() }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <a href="{{ route('landlord.property.show', $property) }}" class="text-blue-600 hover:text-blue-900 mr-3">View</a>
                                <a href="{{ route('landlord.property.edit', $property) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
        ...
        @if($landlord->properties->count() > 5)
    ...
    @else
        <div class="bg-gray-50 p-4 rounded text-gray-500 text-center">
            ...
            <a href="{{ route('landlord.property.create') }}" class="text-blue-600 hover:text-blue-800">Add your first property</a>.
        </div>
    @endif
</div>
@endsection
