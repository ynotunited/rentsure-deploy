...
<div class="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
    <div class="px-4 py-6 max-w-md mx-auto">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-1">Hi, {{ Auth::user()->name }} 👋</h1>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-700">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        @if(Auth::user()->verification_badge)
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-700">
                ...
                <div class="space-y-3">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-gray-900 dark:text-white text-sm font-medium">{{ $approvedDocs }}/{{ $totalDocs }}</span>
                    </div>
                    <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                        <div class="bg-pink-500 h-2 rounded-full" style="width: {{ $totalDocs > 0 ? ($approvedDocs / $totalDocs) * 100 : 0 }}%"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-700">
                ...
                <div class="space-y-3">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-gray-900 dark:text-white text-sm font-medium">{{ $completionScore }}%</span>
                    </div>
                    <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                        <div class="bg-orange-500 h-2 rounded-full" style="width: {{ $completionScore }}%"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-700">
                ...
                <div class="space-y-3">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-gray-900 dark:text-white text-sm font-medium">{{ $trustScore }}%</span>
                    </div>
                    <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                        <div class="bg-yellow-500 h-2 rounded-full" style="width: {{ $trustScore }}%"></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-lg shadow">
        ...
        <div class="p-6">
            @if(Auth::user()->documents()->count() > 0)
                <div class="space-y-4">
                    @foreach(Auth::user()->documents()->latest()->get() as $document)
                        <div class="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    <h4 class="text-sm font-medium text-gray-900">{{ ucfirst(str_replace('_', ' ', $document->type)) }}</h4>
                                    <p class="text-sm text-gray-500">{{ $document->original_name }}</p>
                                    <p class="text-xs text-gray-400">Uploaded {{ $document->created_at->diffForHumans() }}</p>
                                </div>
                            </div>
                            <div class="text-right">
                                ...
                                @if($document->admin_notes)
                                    <p class="text-xs text-gray-500 mt-1">{{ $document->admin_notes }}</p>
                                @endif
                            </div>
                        </div>
                    @endforeach
    ...
    @if(!Auth::user()->verification_badge && !Auth::user()->hasPendingVerificationRequest())
